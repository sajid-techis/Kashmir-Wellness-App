...
export class CreateLabAvailabilityDto {
...
export class LabServiceDto {
    @ApiProperty({ example: 'Complete Blood Count (CBC)', description: 'Name of the service' })
    @IsString()
    @IsNotEmpty()
    name: string;

    @ApiProperty({ example: 500, description: 'Price of the service in INR' })
    @IsNumber()
    @Min(0)
    price: number;
}
